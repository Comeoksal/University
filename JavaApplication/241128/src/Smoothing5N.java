...
public class Smoothing5N{
    ...
    public Smoothing5N(String U_InputFile_Path, String U_OutputFile_Path){
        ...
        for(column = 0; column<=height-1; column++){
            for(row = 0; row<=width-1; row++){
                ...
                green /=count;
                blue /=count;
                Color colorSmooth = new Color(red, green, blue);
                TargetImage.setRGB(row, column, colorSmooth.getRGB());
        }
